...
/**
 * This is a TransferHandler class that is used by the Category JTree
 * to handle DND events.
 * 
 * @author Jesse Saran
 *
 */
public class CategoryDragger extends TransferHandler
{
	...
	/**
	 * Constructor to create a Category Dragger.
	 * 
	 * @param mainModel Instance of the Model.
	 * @param df Instance of the DefualtFrame
	 */
	public CategoryDragger(Model mainModel, DefaultFrame df)
	{
	...
	public boolean importData(TransferSupport support)
	{
		...
		if(root.toString().equals("Categories"))
		{
		...
		node = (DefaultMutableTreeNode) root.getChildAt(childIndex);
		...
		for(int i = 0; i < nodes.size(); ++i)
		{
			if(nodes.get(i).getUserObject() instanceof Group)
			{
				...
				mainModel.addGroupToCategory(((Category) node.getUserObject()).getName(), group.getName());
	...
	public boolean canImport(TransferSupport support)
	{
	    ...
	    if(dropPath == null || !(((DefaultMutableTreeNode) dropPath.getLastPathComponent()).getUserObject() instanceof Category))
	    {
	    	return false;
	    }
		...
		for(int i = 0; i < nodes.size(); ++i)
		{
			...
			//check if the node already is in category
			if(((DefaultMutableTreeNode) dropPath.getLastPathComponent()).getChildCount() > 0)
			{
				DefaultMutableTreeNode sib = (DefaultMutableTreeNode)((DefaultMutableTreeNode) dropPath.getLastPathComponent()).getChildAt(0);
				for(int a = 0; a <= sib.getSiblingCount(); ++a)
				{
					if(nodes.get(i).getUserObject().toString().equals(sib.getUserObject().toString()))
					{
						return false;
					}
	...
	protected void exportDone(JComponent source, Transferable data, int action)
	{
		if(action == MOVE)
		{
			...
			{
				DefaultMutableTreeNode groupSelect = (DefaultMutableTreeNode) temp.getLastPathComponent();
				DefaultMutableTreeNode parent = (DefaultMutableTreeNode) groupSelect.getParent();
				mainModel.removeGroupFromCategory(((Category)parent.getUserObject()).getName(),
						((Group)groupSelect.getUserObject()).getName());
				model.removeNodeFromParent((MutableTreeNode) groupSelect);
			}
			df.update(null,null);
		}
	}
